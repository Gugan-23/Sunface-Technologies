...
const countrySelectBox = document.getElementById('countrySelectBox');
  ...
  for (const country of countries) {
    ...
    li.innerHTML = `
      <div style="display:flex; align-items:center; gap: 8px;">
        <span class="iconify" data-icon="flag:${country.code.toLowerCase()}-4x3" data-inline="false"></span>
        <span>${country.name}</span>
      </div>
      <strong>+${country.phone}</strong>
    `;
    li.dataset.code = country.phone;
    li.dataset.flag = country.code;
    li.addEventListener('click', () => {
      selectedFlag.remove();
      // Set selected values with lowercase code for Iconify
      selectedFlag.setAttribute('data-icon', `flag:${country.code.toLowerCase()}-4x3`);
      const existingFlag = selectedOption.querySelector('.iconify');
existingFlag.setAttribute('data-icon', `flag:${country.code.toLowerCase()}-4x3`);

if (window.Iconify) window.Iconify.scan();

  // Update country code text as usual
  countryCodeElem.textContent = `+${country.phone}`;
  countryOptions.classList.remove('active');
  countrySearch.value = '';
  filterCountries();
  signupPhoneInput.placeholder = `Enter phone number without +${country.phone}`;
...
function setDefaultCountry(defaultCode = 'in') {
  ...
  signupPhoneInput.placeholder = `Enter phone number without +${defaultCountry.phone}`;
  if (window.Iconify) window.Iconify.scan();
}
...
function filterCountries() {
  ...
  const options = countriesList.querySelectorAll('li.option');
  options.forEach(option => {
    const countryName = option.querySelector('div span:nth-child(2)').textContent.toLowerCase();
...
document.getElementById("signupForm").addEventListener("submit", async (e) => {
  ...
  const countryCode = countryCodeElem.textContent.trim();
  ...
  try {
    const res = await fetch(`http://localhost:5000/api/auth/signup`, {
      ...
      body: JSON.stringify({ username, phone: fullPhone, email, password }),
    });
